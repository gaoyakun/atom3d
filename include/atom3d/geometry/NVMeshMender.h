...
------------------------
	...
	vArray< MeshMender::Vertex > theVerts;
	vArray< unsigned int > theIndices;
	vArray< unsigned int > mappingNewToOld;
...
******************************************************************************/
#ifndef __AOTM3D_GEOMETRY_NV_MESH_MENDER_H
#define __AOTM3D_GEOMETRY_NV_MESH_MENDER_H
...
{
	template <>
	struct less<ATOM_Vector3f>
	{
		bool operator () (const ATOM_Vector3f &v1, const ATOM_Vector3f &v2) const
		{
			if (v1.x < v2.x) return true; else if (v1.x > v2.x) return false;
			if (v1.y < v2.y) return true; else if (v1.y > v2.y) return false;
			if (v1.z < v2.z) return true; else if (v1.z > v2.z) return false;
			return false;
		}
	};
}

class ATOM_MeshMender
{
public:
    class Vertex
    {
...
public:
	...
	typedef std::map<ATOM_Vector3f, TriangleList> VertexChildrenMap;
	...
	void SetUpFaceVectors(Triangle& t ,const std::vector<Vertex>&verts, const NormalCalcOption computeNormals);
    ...
    void GetGradients( const ATOM_MeshMender::Vertex& v0,
    ...
    void UpdateTheIndicesWithFinalIndices(std::vector<unsigned>& theIndices);
    void FixCylindricalWrapping(std::vector<Vertex>& theVerts ,
				                std::vector<unsigned>& theIndices,
				                std::vector<unsigned>& mappingNewToOldVert);

    bool TriHasEdge(const ATOM_Vector3f& p0,
...
#endif // __AOTM3D_GEOMETRY_NV_MESH_MENDER_H
