...
* Vital statistics:
  ...
  ATOM_Thread-safety: NOT thread-safe unless USE_LOCKS defined
...
/*
  ...
  struct ATOM_Node { int item; struct ATOM_Node* next; };

  struct ATOM_Node* build_list() {
    struct ATOM_Node** pool;
    int n = read_number_of_nodes_needed();
    if (n <= 0) return 0;
    pool = (struct ATOM_Node**)(independent_calloc(n, sizeof(struct ATOM_Node), 0);
    if (pool == 0) die();
    // organize into a linked list...
    struct ATOM_Node* first = pool[0];
    for (i = 0; i < n-1; ++i)
