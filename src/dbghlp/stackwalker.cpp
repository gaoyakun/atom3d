...
public:
  ...
  ~StackWalkerInternal()
  {
    if (pSC != NULL && m_hProcess != (HANDLE)0xFFFFFFFF)
  ...
  BOOL Init(LPCSTR szSymPath)
  {
    ...
    m_hDbhHelp = ATOM_LoadDebugHelpDll ();
    if (m_hDbhHelp == NULL)
      return FALSE;
...
private:
  ...
  BOOL GetModuleListTH32(HANDLE hProcess, DWORD pid)
  {
    ...
    while (keepGoing)
    {
      this->LoadModule(hProcess, me.szExePath, me.szModule, (DWORD) me.modBaseAddr, me.modBaseSize);
      cnt++;
  ...
  BOOL GetModuleListPSAPI(HANDLE hProcess)
  {
    ...
    for ( i = 0; i < cbNeeded / sizeof hMods[0]; i++ )
    {
      ...
      DWORD dwRes = this->LoadModule(hProcess, tt, tt2, (DWORD) mi.lpBaseOfDll, mi.SizeOfImage);
...
BOOL StackWalker::LoadModules()
{
  ...
  // Build the sym-path:
  char *szSymPath = NULL;
  if ( 1 || (this->m_options & SymBuildPath) != 0)
  {
    ...
    {
      strcat_s(szSymPath, nSymPathLen, "symsrv*symsrv.dll*http://msdl.microsoft.com/download/symbols;");
    }
  }
...
void StackWalker::ResolveFunctionName (CallstackEntry &entry)
{
  if (entry.offset)
  {
    ...
    if (this->m_sw->pSGSFA(this->m_hProcess, (DWORD)entry.offset, &(entry.offsetFromSmybol), pSym) != FALSE)
    {
      // TODO: Mache dies sicher...!
      strncpy(entry.name, pSym->Name, STACKWALK_MAX_NAMELEN);
	  entry.name[STACKWALK_MAX_NAMELEN-1] = '\0';
      // UnDecorateSymbolName()
      this->m_sw->pUDSN( pSym->Name, entry.undName, STACKWALK_MAX_NAMELEN, UNDNAME_NAME_ONLY );
      this->m_sw->pUDSN( pSym->Name, entry.undFullName, STACKWALK_MAX_NAMELEN, UNDNAME_COMPLETE );
    }

    // show line number info, NT5.0-method (SymGetLineFromAddr64())
    if (this->m_sw->pSGLFA != NULL )
    { // yes, we have SymGetLineFromAddr64()
      if (this->m_sw->pSGLFA(this->m_hProcess, (DWORD)entry.offset, &(entry.offsetFromLine), &Line) != FALSE)
      {
        entry.lineNumber = Line.LineNumber;
        // TODO: Mache dies sicher...!
        strcpy_s(entry.lineFileName, Line.FileName);
      }
    } // yes, we have SymGetLineFromAddr64()

    // show module info (SymGetModuleInfo64())
    if (this->m_sw->GetModuleInfo(this->m_hProcess, (DWORD)entry.offset, &Module ) != FALSE)
    { // got module info OK
      // TODO: Mache dies sicher...!
      strcpy_s(entry.moduleName, Module.ModuleName);
      entry.baseOfImage = Module.BaseOfImage;
      strcpy_s(entry.loadedImageName, Module.LoadedImageName);
    } // got module info OK
...
BOOL StackWalker::ShowCallstack(HANDLE hThread, const CONTEXT *context, PReadProcessMemoryRoutine readMemoryFunction, LPVOID pUserData)
{
  ...
  for (frameNum = 0; ; ++frameNum )
  {
	...
	if (s.AddrPC.Offset > std::numeric_limits<UINT_PTR>::max ())
	{
		break;
	}

    csEntry.offset = (void*)(DWORD)s.AddrPC.Offset;
    csEntry.name[0] = 0;
    csEntry.undName[0] = 0;
    ...
    if (s.AddrPC.Offset != 0 && (m_options & StackWalker::RetrieveSymbol) != 0)
    {
      IMAGEHLP_SYMBOL64 *pSym = (IMAGEHLP_SYMBOL64 *) malloc(sizeof(IMAGEHLP_SYMBOL64) + STACKWALK_MAX_NAMELEN);
      memset(pSym, 0, sizeof(IMAGEHLP_SYMBOL64) + STACKWALK_MAX_NAMELEN);
      pSym->SizeOfStruct = sizeof(IMAGEHLP_SYMBOL64);
    ...
    )
{
  ...
  {
    SIZE_T st;
    BOOL bRet = ReadProcessMemory(hProcess, (LPVOID)(DWORD)qwBaseAddress, lpBuffer, nSize, &st);
