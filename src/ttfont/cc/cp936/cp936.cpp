...
int ATOM_CALL ATOM_CC_GetMaxByteCount_CP936 ()
{
return MaxByteCount ();
}
...
int ATOM_CALL ATOM_CC_ConvertTextToUnicode_CP936 (ATOM_UNICC *dst, const char *src, 
        ...
        if (IsLeadByte (*src_buffer))
          {
            ...
            if (count_conv > 1)
              {
                *dst++ = ATOM_CC_ConvertCharToUnicode_CP936 (*((ATOM_MBCC*)src_buffer));
                src_buffer += 2;
                count_conv -= 2;
                --DstBufferLen;
              }
...
int ATOM_CALL ATOM_CC_ConvertTextToMultibyte_CP936 (ATOM_MBCC *dst, const char *src, 
                                 int SrcBufferLen, int DstBufferLen)
{
if (src == 0)
  return 0;
        ...
        if (IsLeadByte (*src_buffer))
          {
            ...
            if (count_conv > 1)
              {
                *dst++ = swap_byte(*((ATOM_MBCC*)src_buffer));
                src_buffer += 2;
                count_conv -= 2;
                --DstBufferLen;
              }
...
{
  static const ATOM_UNICC uniccUnknown = ATOM_CC_ConvertCharToUnicode_CP936 ('?');
  ...
  else
  {
    if (IsLeadByte (*str))
    {
      if (str + 1 < strEnd)
      {
         *ch = ATOM_CC_ConvertCharToUnicode_CP936 (*((ATOM_MBCC*)str));
         return str + 2;
      }
      else
      {
...
const char * ATOM_CALL ATOM_CC_GetPrevUNIATOM_CC_CP936 (const char *str, const char *strEnd, ATOM_UNICC *ch)
{
  ...
  else
  {
	...
	{
		*ch = ATOM_CC_ConvertCharToUnicode_CP936 (*(strEnd-1));
		return strEnd - 1;
	}
	else
	{
		*ch = ATOM_CC_ConvertCharToUnicode_CP936 (*((ATOM_MBCC*)(strEnd-2)));
		return strEnd - 2;
	}
  }
}
...
const char * ATOM_CALL ATOM_CC_GetPrevMBATOM_CC_CP936 (const char *str, const char *strEnd, ATOM_MBCC *ch)
{
  ...
  {
    if (str + 1 == strEnd || !IsLeadByte (*(strEnd-1)))
	{
		*ch = *(strEnd-1);
		return strEnd - 1;
	}
