...
ATOM_DDB_DisplayInfo::ATOM_DDB_DisplayInfo (void)
{
  ...
  {
    DisplayDeviceInfo *di = new DisplayDeviceInfo;
    ...
    if( FAILED( hr = GetStringValue( pObject, L"szDisplayModeLocalized", di->m_strDisplayMode, ARRAY_SIZE(di->m_strDisplayMode))))
      goto LCleanup;
...
ATOM_DDB_DisplayInfo::~ATOM_DDB_DisplayInfo ()
{
...
unsigned ATOM_DDB_DisplayInfo::GetNumDevices (void) const
{
  return devices.size();
}

const char *ATOM_DDB_DisplayInfo::GetDeviceDescription (unsigned device) const
{
  return devices[device]->m_strDescription;
}

const char *ATOM_DDB_DisplayInfo::GetDeviceManufacturer (unsigned device) const
{
  return devices[device]->m_strManufacturer;
}

const char *ATOM_DDB_DisplayInfo::GetDeviceChipType (unsigned device) const
{
  return devices[device]->m_strChipType;
}

const char *ATOM_DDB_DisplayInfo::GetDeviceMemory (unsigned device) const
{
  return devices[device]->m_strDisplayMemory;
}

const char *ATOM_DDB_DisplayInfo::GetDeviceDisplayMode (unsigned device) const
{
  return devices[device]->m_strDisplayMode;
}

const char *ATOM_DDB_DisplayInfo::GetDeviceDriverName (unsigned device) const
{
  return devices[device]->m_strDriverName;
}

const char *ATOM_DDB_DisplayInfo::GetDeviceDriverVersion (unsigned device) const
{
  return devices[device]->m_strDriverVersion;
}

const char *ATOM_DDB_DisplayInfo::GetDeviceDriverDate (unsigned device) const
{
  return devices[device]->m_strDriverDate;
}

unsigned ATOM_DDB_DisplayInfo::GetDeviceVendorId (unsigned device) const
{
	return strtol (devices[device]->m_strVendorId, 0, 16);
}

unsigned ATOM_DDB_DisplayInfo::GetDeviceId (unsigned device) const
{
	return strtol (devices[device]->m_strDeviceId, 0, 16);
}

unsigned ATOM_DDB_DisplayInfo::GetDeviceSubSystemId (unsigned device) const
{
	return strtol (devices[device]->m_strSubSysId, 0, 16);
}

unsigned ATOM_DDB_DisplayInfo::GetDeviceRevisionId (unsigned device) const
{
	return strtol (devices[device]->m_strRevisionId, 0, 16);
}

bool ATOM_DDB_DisplayInfo::IsDDrawAccelerationEnabled (unsigned device) const
{
  return devices[device]->m_bDDAccelerationEnabled;
}

bool ATOM_DDB_DisplayInfo::Is3DAccelerationExists (unsigned device) const
{
  return devices[device]->m_b3DAccelerationExists;
}

bool ATOM_DDB_DisplayInfo::Is3DAccelerationEnabled (unsigned device) const
{
  return devices[device]->m_b3DAccelerationEnabled;
}

bool ATOM_DDB_DisplayInfo::IsAGPExists (unsigned device) const
{
  return devices[device]->m_bAGPExists;
}

bool ATOM_DDB_DisplayInfo::IsAGPEnabled (unsigned device) const
{
  return devices[device]->m_bAGPEnabled;
}

const ATOM_DDB_DisplayInfo::DxVersionInfo *ATOM_DDB_DisplayInfo::GetDxVersionInfo (void) const
{
  return &dxverion;
}

const char *ATOM_DDB_DisplayInfo::GetDXVersionString (void) const
{
