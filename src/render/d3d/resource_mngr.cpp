...
int CResourceReuseCache::EnsureFreeTexture( UINT Width, UINT Height, UINT MipLevels, UINT Format )
{
    ...
    if( !m_bDontCreateResources )
    {
        ...
        if( FAILED( hr ) )
        {
            SAFE_DELETE( tex );
            if( !m_bSilent )
                OutputDebugString( "RESOURCE ERROR: Cannot Load Texture!\n" );
            return -1;
        }
...
int CResourceReuseCache::EnsureFreeVB( UINT iSizeBytes )
{
    ...
    if( !m_bDontCreateResources )
    {
        ...
        if( FAILED( hr ) )
        {
            SAFE_DELETE( vb );
            if( !m_bSilent )
                OutputDebugString( "RESOURCE ERROR: Cannot Load Vertex Buffer!\n" );
            return -1;
        }
...
int CResourceReuseCache::EnsureFreeIB( UINT iSizeBytes, UINT ibFormat )
{
    ...
    if( !m_bDontCreateResources )
    {
        ...
        if( FAILED( hr ) )
        {
            SAFE_DELETE( IB );
            if( !m_bSilent )
                OutputDebugString( "RESOURCE ERROR: Cannot Load Index Buffer!\n" );
            return -1;
        }
