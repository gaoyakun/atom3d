...
void MaterialViewer::addMaterials (const MObjectArray &shaders)
{
	for (unsigned i = 0; i < shaders.length(); ++i)
	{
		MPlug mShader = MFnDependencyNode(shaders[i]).findPlug ("surfaceShader");
		if (mShader.isNull ())
		{
			continue;
		}
		...
		const char *name = MFnDependencyNode(plugs[0].node()).name().asChar();
