...
void ATOM_TiXmlElement::SetAttribute( const char * cname, const char * cvalue )
{
	...
	ATOM_TiXmlAttribute* attrib = new ATOM_TiXmlAttribute( cname, cvalue );
	if ( attrib )
	{
...
void ATOM_TiXmlElement::SetAttribute( const std::string& name, const std::string& _value )
{
	...
	ATOM_TiXmlAttribute* attrib = new ATOM_TiXmlAttribute( name, _value );
	if ( attrib )
	{
		attributeSet.Add( attrib );
	}
...
{
	ATOM_TiXmlElement* clone = new ATOM_TiXmlElement( Value() );
	if ( !clone )
		return 0;
...
{
	ATOM_TiXmlDocument* clone = new ATOM_TiXmlDocument();
	if ( !clone )
		return 0;
...
{
	ATOM_TiXmlComment* clone = new ATOM_TiXmlComment();
...
{	
	ATOM_TiXmlDeclaration* clone = new ATOM_TiXmlDeclaration();
...
{
	ATOM_TiXmlUnknown* clone = new ATOM_TiXmlUnknown();
