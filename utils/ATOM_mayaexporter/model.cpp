	...
	bool write (FILE *fp)
	{
		if (sklt)
		{
			...
			for (unsigned s = 0; s < numSkeletons; ++s)
			{
				...
				{
					char buffer[64];
					strcpy (buffer, sklt->getJoints()[newToOld[m]].name.asChar());
		...
		for (unsigned a = 0; a < numActions; ++a)
		{
			...
			for (unsigned f = 0; f < numFrames; ++f)
			{
				...
				for (unsigned m = 0; m < numMeshes; ++m)
				{
					...
					actualWriteSize += fwrite (&vaf.vertexCacheIndex, 1, sizeof(int), fp);
					actualWriteSize += fwrite (&vaf.uvCacheIndex, 1, sizeof(int), fp);
...
bool MyModel::save (const char *filename, bool saveSkeleton, bool saveAnimations, bool saveMeshes, SkeletonViewer *viewer, PropertyEditor *props, VertexAnimationInfo *animationInfo)
{
	if (meshes().size() > 0)
	{
		...
		if (meshes().empty ())
		{
			::MessageBox (::GetActiveWindow(), "导出失败!", "atom exporter", MB_OK|MB_ICONHAND);
			return false;
		}
	}
