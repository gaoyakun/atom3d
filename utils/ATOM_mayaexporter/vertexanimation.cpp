...
{
	MTime mtStart = MAnimControl::animationStartTime ();
	MTime mtEnd = MAnimControl::animationEndTime ();
	MTime mtInc (MAnimControl::playbackBy (), MTime::uiUnit());

	info->msStart = mtStart.as (MTime::kMilliseconds);
	info->msEnd = mtEnd.as (MTime::kMilliseconds);
	info->msInc = mtInc.as (MTime::kMilliseconds);
