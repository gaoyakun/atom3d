...
bool SkeletonViewer::setSkeleton (ATOM_Skeleton *skeleton, bool selectBoneByName)
{
...
void SkeletonViewer::initTree (HWND treeCtrl)
{
	...
	for (unsigned i = 0; i < skeleton->getJoints().size(); ++i)
	{
		const ATOM_Skeleton::joint &theJoint = skeleton->getJoints()[i];
		...
		tvis.item.pszText = (LPSTR)theJoint.name.c_str();
