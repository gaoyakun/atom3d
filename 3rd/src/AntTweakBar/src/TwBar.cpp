...
void CTwBar::Update()
{
    ...
    Gr->BuildText(m_TitleTextObj, &s, NULL, NULL, 1, m_Font, 0, 0);
    ...
    if( !m_IsMinimized )
    {
        ...
        if( Labels.size()>0 )
		{
			vector<const char*> s(Labels.size());
			for (unsigned n = 0; n < s.size(); ++n)
			{
				s[n] = Labels[n].c_str();
			}
            Gr->BuildText(m_LabelsTextObj, &(s[0]), &(Colors[0]), &(BgColors[0]), (int)Labels.size(), m_Font, 1, HasBgColors ? m_VarX1-m_VarX0-m_Font->m_CharHeight+2 : 0);
		}
        else
        ...
        */
        ...
        if( Values.size()>0 )
		{
			vector<const char*> s(Values.size());
			for (unsigned n = 0; n < s.size(); ++n)
			{
				s[n] = Values[n].c_str();
			}
            Gr->BuildText(m_ValuesTextObj, &(s[0]), &(Colors[0]), &(BgColors[0]), (int)Values.size(), m_Font, 1, m_VarX2-m_VarX1);
		}
        else
		...
		const char *s = Shortcut.c_str();
        Gr->BuildText(m_ShortcutTextObj, &s, NULL, NULL, 1, m_Font, 0, 0);
    }
        ...
        else if( m_IsHelpBar )
        {
            if( g_TwMgr->m_KeyPressedTextObj && g_TwMgr->m_KeyPressedStr.size()>0 ) // Draw key pressed
            {
                if( g_TwMgr->m_KeyPressedBuildText )
                {
                    ...
                    g_TwMgr->m_Graph->BuildText(g_TwMgr->m_KeyPressedTextObj, &s, NULL, NULL, 1, g_TwMgr->m_HelpBar->m_Font, 0, 0);
            ...
            else
            {
                if( g_TwMgr->m_InfoBuildText )
                {
                    ...
                    g_TwMgr->m_Graph->BuildText(g_TwMgr->m_InfoTextObj, &s, NULL, NULL, 1, g_TwMgr->m_HelpBar->m_Font, 0, 0);
...
bool CTwBar::CharEntered(unsigned short mbcc)
{
    assert(g_TwMgr->m_Graph && g_TwMgr->m_WndHeight>0 && g_TwMgr->m_WndWidth>0);
    bool Handled = false;
    if( !m_UpToDate )
    ...
    if( m_EditInPlace.m_Active )
    {
        Handled = EditInPlaceCharEntered(mbcc);
    }
    return Handled;
}
...
void CTwBar::EditInPlaceDraw()
{
    ...
    g_TwMgr->m_Graph->BuildText(m_EditInPlace.m_EditTextObj, &s, NULL, NULL, 1, m_Font, 0, m_EditInPlace.m_Width);
    ...
    if( SelWidth>0 && StrSelected.length()>0 )
    {
		...
		const char *s = StrSelected.c_str();
        g_TwMgr->m_Graph->BuildText(m_EditInPlace.m_EditSelTextObj, &s, NULL, NULL, 1, m_Font, 0, SelWidth);
        if ( m_EditInPlace.m_CaretPos>m_EditInPlace.m_SelectionStart )
...
bool CTwBar::EditInPlaceCharEntered(unsigned short mbcc)
{
	if( !m_EditInPlace.m_Active )
		return false;
	...
	default:
		if( !EditInPlaceIsReadOnly() && m_EditInPlace.m_CaretPos>=0 && m_EditInPlace.m_CaretPos<=(int)m_EditInPlace.m_String.length() )
		{
			...
			string Str;
			char leadByte = (char)(mbcc & 0xFF);
			char nextByte = (char)((mbcc >> 8) & 0xFF);
			Str += leadByte;
			int inc = 1;
			if (nextByte)
			{
