...
void CTwGraphOpenGL::BuildText(void *_TextObj, const char**_TextLines, color32 *_LineColors, color32 *_LineBgColors, int _NbLines, const CTexFont *_Font, int _Sep, int _BgWidth)
{
    assert(m_Drawing==true);
    assert(_TextObj!=NULL);
    assert(_Font!=NULL);
...
void CTwGraphOpenGL::DrawTriangles(int _NumTriangles, int *_Vertices, color32 *_Colors, Cull _CullMode)
{
    ...
    int prevCullFaceMode, prevFrontFace;
...
void CTwGraphOpenGL::DeleteThis()
{
	delete this;
}
