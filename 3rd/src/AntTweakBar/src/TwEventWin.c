    ...
    case WM_SYSCHAR:
        {
			...
			if (IsWindowUnicode ((HWND)wnd))
			{
				// 对于Unicode窗体,WM_CHAR里的WPARAM保存字符的Unicode编码
				// 转换成MultiByte先
				unsigned short unicode = wParam;
				WideCharToMultiByte (CP_ACP, 0, (LPCWSTR)&unicode, 1, (LPSTR)&key, 2, 0, 0);
			}
			else
			{
				key = (unsigned short)(wParam&0xff);
			}

			if (key > 32)
			{
				handled = TwCharEntered(key);
			}
        }
        break;
    case WM_KEYDOWN:
