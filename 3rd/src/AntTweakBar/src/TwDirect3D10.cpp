...
void CTwGraphDirect3D10::BuildText(void *_TextObj, const char **_TextLines, color32 *_LineColors, color32 *_LineBgColors, int _NbLines, const CTexFont *_Font, int _Sep, int _BgWidth)
{
    assert(m_Drawing==true);
    assert(_TextObj!=NULL);
    assert(_Font!=NULL);
    ...
    for( line=0; line<_NbLines; ++line )
        nbTextVerts += 6 * (int)strlen(_TextLines[line]);
    int nbBgVerts = 0;
    if( _BgWidth>0 )
...
void CTwGraphDirect3D10::DeleteThis()
{
	delete this;
}
